...
func main() {
	...
	rawRoot := flag.String("raw-root", `history_external/raw/binance/futures/um/daily/metrics/BTCUSDT`, "raw Binance daily metrics ZIP root")
	phase2 := flag.Bool("phase2", false, "publish Phase 2 change-point, raw/canonical, and leakage reports")
	...
	if *phase2 {
		if err := writePhase2Reports(*output, dates, all, *rawRoot, *metricsRoot); err != nil {
			fatal(err)
		}
	}
